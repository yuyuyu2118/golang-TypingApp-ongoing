...
func battleTypingSkill(win *pixelgl.Window, player *PlayerStatus, enemy *EnemyStatus) {
...
func deathFlug(player *PlayerStatus, enemy *EnemyStatus, elapsed time.Duration, currentGameState GameState) GameState {
	if player.playerHP <= 0 {
		yourTime = float64(elapsed.Seconds())
		min := int(float64(enemy.enemyGold) * 0.7)
		max := int(float64(enemy.enemyGold) * 1.3)
